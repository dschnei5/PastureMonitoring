...
img.nms.dtes <- function(propmeta = "Propmeta", x = "Property Name", out.dir, skip) {
  ...
  if (skip){
    already.done <- list.files(path = paste(out.dir,"/GeoJson"), pattern = ".geojson", recursive = FALSE);
...
post.processor <- function() {
  t1 <- now()
  print(paste(now(),"- Processing", x, "started"));
  propmeta.i <- setup.post.dirs(propmeta, x);
  out.dir <- paste0(d.dir,propmeta.i$OutLoc);
  image.names.dates <- img.nms.dtes(propmeta, x, skip = skip);
  ...
  if (length(img.names) == 0) {
      ...
      boundary1 <- shps[[propmeta.i$Shapefile %in% names(shps)]]
      ...
      for (i in seq_along(img.dates)){
        ...
        ndvi.crp1 <- intersect(extent(ndvi), dims1);
        ...
        if (length(ndvi.crp1) == 0 | ndvi.crp2 == FALSE | class(boundary1)[1]=="SpatialPointsDataFrame") {
          message("No overlap or it's a point file, moving on...")
          if(class(boundary1)[1]=="SpatialPointsDataFrame") {
            message("Ahah! it's a point file...")
            Sys.sleep(1);
            message("Performing spatial intersection...")
            treemask2 <- raster(paste0(wd,propmeta.i$TreeMask));
            ...
            colnames(dta.out2) <- c(paste0( "Sent_NDVI_",img.dates[j]),paste0("GDM_Est_",img.dates[j]));
            ...
            write.csv(dta.out1, paste0(wd,propmeta.i$OutLoc,"/", x[i], "_GDM_",img.dates[j],".csv"));
        ...
        message(paste("Time Elapsed =", time_length(Sys.time() - strt.tm), "seconds"));
        Sys.sleep(1);
        suppressWarnings(rm(strt.tm, ndvi.crp1,ndvi.crp2,ndvi));
        gc();
        gc();
      };# END i Loop
      rm(i)
      if (skip == TRUE) {
        ...
        if (dof.len >= 6) {
          ...
          message( "All new files contained imagery that didnt overlap with property shapefile...")
        }
        
        
        if(R.Version()[[7]] == "1.0" & dof.len >= 6) {
          boundary2 <- boundary1;
          if (class(boundary1)[1]=="SpatialPointsDataFrame"){
          ...
          if (class(boundary1)[1]=="SpatialPointsDataFrame"){
            geojsonio::geojson_write(boundary2, geometry = "point", file = paste0(out.dir,"/GeoJson/",x[i],"_GDM_AllAvailableDates.geojson"), convert_wgs84 = FALSE, crs = UCRS[[propmeta.i$CRS.no]]);
          } else {
            geojsonio::geojson_write(boundary2, geometry = "polygon", file = paste0(out.dir,"/GeoJson/",x[i],"_GDM_AllAvailableDates.geojson"), convert_wgs84 = FALSE, crs = UCRS[[propmeta.i$CRS.no]]);
          };
          suppressWarnings(file.copy(from = paste0(out.dir,"/GeoJson/",x[i],"_GDM_AllAvailableDates.geojson"), to = paste0(wd,propmeta.i$AppFile,"/",x[i],"/GeoJson/",x[i],"_GDM_AllAvailableDates.geojson"), overwrite = TRUE, copy.date = TRUE));
          if (!is.na(propmeta.i$AppFile2nd)) {suppressWarnings(file.copy(from = paste0(out.dir,"/GeoJson/",x[i],"_GDM_AllAvailableDates.geojson"), to = paste0(wd,propmeta.i$AppFile2nd,"/",x[i],"/GeoJson/",x[i],"_GDM_AllAvailableDates.geojson"), overwrite = TRUE, copy.date = TRUE))};
          suppressWarnings(file.copy(from = paste0(out.dir,"/GeoJson/",x[i],"_GDM_AllAvailableDates.geojson"), to = paste0(wd,"/ShinyApps",master.app,"/www/DataOut/",x[i],"/GeoJson/",x[i],"_GDM_AllAvailableDates.geojson"), overwrite = TRUE, copy.date = TRUE));
      ...
      Sys.sleep(1);
      message(paste("Processing end time:", Sys.time()));
      Sys.sleep(2);
  ...
  rm(dta.out.full,boundary1, boundary2)
  gc();
  gc();
  }
...
postprocess.fast <- function(x) {
  ...
  getDoParWorkers();
  foreach(i = seq_along(x), .packages=c('lubridate','raster', 'rgdal','plyr','doParallel','gsubfn'), .export = c(ls(e),ls(.GlobalEnv))) %dopar% {
    post.processor()
  };
  stopImplicitCluster();
};
...
md5post <- function(av.shiny.apps, MD5.check1) {
  ...
  MD5.check4$AppFile <- pm$AppFile[pm$Property  %in%  MD5.check4$property2]
  MD5.check4$AppFile <- gsub("/www/DataOut","",MD5.check4$AppFile);
  MD5.check4$AppFile2nd <- propmeta$AppFile2nd[pm$Property  %in%  MD5.check4$property2]
